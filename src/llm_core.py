...
def _local_model_gate_enabled() -> bool:
    return os.getenv("AEGIS_LOCAL_MODEL_GATE", "true").lower() not in {"0", "false", "no", "off"}
...
def _apply_local_cache_affinity(payload: Dict, url: str, session_id: Optional[str]) -> None:
    ...
    Sending a stable ``session_id`` (derived from the Aegis session) lets
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://aegis.local")
...
# value returns HTTP 400. Aegis does not currently send the `thinking` mode
...
# AEGIS_MISTRAL_REASONING_EFFORT (e.g. set to "medium" for cheaper chat).
_MISTRAL_REASONING_EFFORT = os.getenv("AEGIS_MISTRAL_REASONING_EFFORT", "high")
...
async def _stream_llm_inner(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
    ...
    if provider == "anthropic":
        ...
        # Effort level is configurable via AEGIS_MISTRAL_REASONING_EFFORT
