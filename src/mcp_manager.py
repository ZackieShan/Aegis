...
class McpManager:
    ...
    async def connect_all_enabled(self):
        ...
        try:
            ...
            for srv in servers:
                # Built-in toolboxes have McpServer rows (so they show in the
                # admin panel) but are connected by register_builtin_servers.
                # Skip them here — reconnecting a live stdio server tears down
                # its anyio task group from a different task and raises a noisy
                # "exit cancel scope in a different task" RuntimeError.
                if srv.id in BUILTIN_MCP_IDS:
                    continue
                # Belt-and-suspenders: never re-drive an already-connected server.
                if self._connections.get(srv.id, {}).get("status") == "connected":
                    continue
