...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- Aegis chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
def _endpoint_lookup_keys(endpoint_url: str) -> List[str]:
    ...
    try:
        ...
        add(nb)
        # Ollama endpoints are commonly stored as the bare host
        # (http://host:port) while the runtime chat URL is
        # http://host:port/api/chat. normalize_base only strips the trailing
        # /chat, leaving a /api (or /v1) segment, so the bare-host row never
        # matches. Also try the host root so per-endpoint flags like
        # supports_tools resolve for those endpoints.
        for suf in ("/api", "/v1"):
            if nb.endswith(suf):
                add(nb[: -len(suf)])
    except Exception:
...
def _classify_agent_request(messages: List[Dict], last_user: str) -> Dict[str, object]:
    ...
    if has(r"\b(cookbook|serve|serving|served|launch|start|preset|vllm|sglang|llama\.?cpp|ollama|download|downloading|pull|cached models?|running models?|model servers?|models? (?:are )?running|what models?|model picker|gpu box|kierkegaard|aegis|ajax|qwen|gemma|llama|mistral|minimax)\b"):
...
def _minimal_saved_memory_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    if not facts:
        return None
    logger.info("[agent-intent] aegis doc minimal memory facts=%s", len(facts))
    return {
        "role": "user",
        "content": (
            "Saved user memory facts from Aegis Brain. These are the same "
...
def _minimal_aegis_doc_messages(messages: List[Dict], active_document, stream_create: bool = False) -> List[Dict]:
    """Tiny prompt path for the Aegis document LoRA.
    ...
    if stream_create:
        system = (
            "You are Aegis. Create the requested document by streaming exactly one fenced block:\n"
            "```document\n"
            "Title\n"
            "markdown\n"
            "Document content\n"
        ...
        system = (
            "You are Aegis. Edit or suggest changes to the active document using exactly one fenced tool block when needed.\n"
            ...
            "After the tool succeeds, Aegis will answer Done."
        )
...
def _minimal_aegis_notes_messages(messages: List[Dict]) -> List[Dict]:
    """Tiny prompt path for Aegis notes LoRAs.

    The finetune is trained to emit Aegis note tool calls without receiving
    the full tool schema or saved-context wrapper stack.
    """
    latest = _extract_last_user_message(messages)
    system = (
        "You are Aegis. Handle note, todo, checklist, and reminder requests.\n"
        "You have access to the user's Aegis notes through manage_notes.\n"
        "For 'what are my notes', 'show my notes', note searches, note creation, todos, checklists, and reminders, use the Aegis manage_notes tool call format.\n"
...
def _minimal_aegis_general_messages(messages: List[Dict], include_memory: bool = False) -> List[Dict]:
    """Minimal fallback for Aegis finetunes outside domain-specific paths."""
    latest = _extract_last_user_message(messages)
    system = (
        "You are Aegis. Answer directly and briefly.\n"
        "Use Aegis tool-call format only when the user explicitly asks you to take an action.\n"
    ...
    if active_document:
        ...
        if _is_email_doc:
            ...
            doc_ctx = (
                ...
                f'2. Replace ONLY the new reply text above `---------- Previous message ----------`. You may omit the quoted history from your tool output; Aegis preserves everything from that separator downward automatically.\n'
...
async def stream_agent_loop(
    ...
    active_toolboxes: Optional[Set[str]] = None,
    ...
    _ody_qwen_finetune_model = (model or "").lower().startswith("aegis-qwen3")
    ...
    # Toolbox servers (OSINT / market / troubleshoot) are opt-in: hide all their
    # tools from the model UNLESS this turn explicitly summoned them via a slash
    # command (active_toolboxes). Without this, their tools flood every request
    # and the model turns ordinary chat into an investigation. Reuses the
    # existing disabled_map plumbing (schemas + prompt both honor it).
    if mcp_mgr:
        try:
            from src.builtin_mcp import TOOLBOX_MCP_IDS
            _active_tb = {t.strip().lower() for t in (active_toolboxes or set()) if t.strip()}
            for _t in mcp_mgr.get_all_tools():
                _sid = _t.get("server_id")
                if _sid in TOOLBOX_MCP_IDS and _sid not in _active_tb:
    ...
    if _direct_low_signal:
        ...
        direct_messages = (
            _minimal_aegis_general_messages(
                messages,
                include_memory=True,
            )
    ...
    if _ody_doc_finetune_mode and _relevant_tools is not None:
        ...
        logger.info("[agent-intent] aegis doc finetune tool clamp=%s", sorted(_relevant_tools))
    elif _ody_notes_finetune_mode and _relevant_tools is not None:
        _relevant_tools = {"manage_notes", "ask_user", "update_plan"}
        logger.info("[agent-intent] aegis notes finetune tool clamp=%s", sorted(_relevant_tools))
    ...
    if _ody_doc_finetune_mode and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_aegis_doc_messages(
            messages,
        ...
        mcp_schemas = []
        logger.info(
            "[agent-intent] aegis doc minimal prompt active active_doc=%s stream_create=%s messages=%s",
        ...
        messages = _minimal_aegis_notes_messages(messages)
        mcp_schemas = []
        logger.info(
            "[agent-intent] aegis notes minimal prompt active messages=%s",
            len(messages),
        )
    elif _ody_qwen_finetune_model and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_aegis_general_messages(
            messages,
            include_memory=True,
        )
        mcp_schemas = []
        logger.info(
            "[agent-intent] aegis general minimal prompt active include_memory=%s messages=%s",
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if _ody_doc_stream_create_mode and tool_blocks:
            ...
            if create_idx is None:
                logger.info(
                    "[agent] aegis doc stream-create discarded non-create tool call(s): %s",
                ...
                if len(tool_blocks) > 1 or create_idx != 0:
                    logger.info(
                        "[agent] aegis doc stream-create keeping first create_document and dropping extras: %s",
                        [block.tool_type for block in tool_blocks],
                    )
        ...
        if _ody_qwen_finetune_model and tool_blocks:
            ...
            if _dropped_memory_lookup:
                logger.info(
                    "[agent-intent] aegis qwen dropped manage_memory lookup; answering from compact memory"
                )
        ...
        if _doc_stream_create_completed:
            ...
            logger.info("[agent] aegis doc stream-create completed after one create_document")
            break
        ...
        if _ody_doc_tool_completed:
            if not full_response.strip() or full_response.strip().startswith("```"):
                full_response = "Done."
                yield 'data: ' + json.dumps({"delta": "Done."}) + '\n\n'
            logger.info("[agent] aegis doc tool completed after one textual tool block")
            break

        if _ody_notes_finetune_mode and _ody_notes_tool_completed:
            logger.info("[agent] aegis notes completed from deterministic tool output")
            break
