...
_BUILTIN_SERVERS = {
    ...
    # "Toolbox" servers — themed collections of callable tools the chat can use.
...
MCP_DISABLED = os.environ.get("AEGIS_DISABLE_MCP", "").lower() in ("1", "true", "yes")
...
# Ids of the built-in MCP servers, so routes can distinguish them from
# admin-added external servers (e.g. block deletion, show a "built-in" badge).
BUILTIN_MCP_IDS = set(_BUILTIN_SERVERS) | set(_BUILTIN_NPX_SERVERS)

# "Toolbox" servers are opt-in: their tools are excluded from normal chat and
# only offered to the model when the user explicitly summons them with a slash
...
def _upsert_builtin_server_row(server_id, name, command, args, env) -> bool:
    """Ensure a McpServer row exists for a built-in server so it appears in the
    admin panel. Returns whether the server is enabled (honoring an existing
    row's is_enabled). Never flips an admin's explicit disable back on."""
    import json as _json
    try:
        from core.database import SessionLocal, McpServer
    except Exception:
        return True  # DB unavailable — connect anyway, just no UI row
    db = SessionLocal()
    try:
        row = db.query(McpServer).filter(McpServer.id == server_id).first()
        if row is None:
            row = McpServer(
                id=server_id, name=name, transport="stdio",
                command=command, args=_json.dumps(args),
                env=_json.dumps(env), is_enabled=True,
            )
            db.add(row)
            db.commit()
            return True
        # Keep command/args/env fresh (paths can change), preserve is_enabled.
        row.name = name
        row.command = command
        row.args = _json.dumps(args)
        row.env = _json.dumps(env)
        db.commit()
        return bool(row.is_enabled)
    except Exception as e:
        logger.debug(f"builtin MCP row upsert failed for {server_id}: {e}")
        return True
    finally:
        db.close()
...
async def register_builtin_servers(mcp_manager):
    ...
    if MCP_DISABLED:
        logger.info("Built-in MCP servers disabled via AEGIS_DISABLE_MCP")
        return
    ...
    for server_id, (script, name) in _BUILTIN_SERVERS.items():
        ...
        if not os.path.exists(script_path):
            ...
            continue
        # Upsert a McpServer row so built-in servers (memory/rag/email + the
        # toolboxes) show up in the admin MCP panel and reuse its enable/disable
        # + per-tool machinery. A pre-existing row's is_enabled is honored, so a
        # toolbox the admin disabled stays disabled across restarts.
        enabled = _upsert_builtin_server_row(
            server_id, name, python, [script_path], {"PYTHONPATH": base_dir}
        )
        if not enabled:
            logger.info(f"Built-in MCP server '{name}' is disabled by admin — skipping connect.")
            continue
