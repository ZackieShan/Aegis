...
class VectorRAG:
    ...
    def remove_directory(self, directory: str) -> Dict[str, Any]:
        ...
        # Normalize both sides before comparing: Windows paths mix / and \
        # and compare case-insensitively, so a raw string match would leave
        # stale chunks behind. normcase/abspath are no-ops on POSIX.
        dir_key = os.path.normcase(directory)

        def _under_dir(source: str) -> bool:
            src_key = os.path.normcase(os.path.abspath(source))
            return src_key == dir_key or src_key.startswith(dir_key + os.sep)
