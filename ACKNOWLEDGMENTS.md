...
Aegis is a derivative of
**[Odysseus](https://github.com/pewdiepie-archdaemon/odysseus)**
(AGPL-3.0-or-later). The original authors retain copyright over the code
inherited from that project; this repository is distributed under the same
...
| Font | License | Author |
|---|---|---|
| [JetBrains Mono](https://github.com/JetBrains/JetBrainsMono) ([`licenses/JetBrainsMono-OFL.txt`](licenses/JetBrainsMono-OFL.txt)) | SIL Open Font License 1.1 | JetBrains |
...
| **PyMuPDF** *(PDF viewer + form-filling)* | **AGPL-3.0** — same license as Aegis; see note below |
...
- **PDF text extraction** now uses **`pypdf`** (BSD-3-Clause) and **encoding
  ...
  removed entirely.
- **PyMuPDF (AGPL-3.0)** is a **core dependency** in Aegis (`requirements.txt`),
  powering the PDF viewer and form-filling (`src/pdf_runtime.py`,
  `src/pdf_forms.py`, `routes/document_routes.py`). Its AGPL license is a
  non-issue here because **Aegis is itself AGPL-3.0-or-later** — the same terms.
  (The upstream project kept it optional to preserve an MIT-compatible core;
  that goal does not apply to this AGPL fork.) PDF *text* extraction still uses
  `pypdf` (BSD); PyMuPDF adds page rendering.
...
Most of the original Odysseus codebase was written *with* AI models, not just by a human.
