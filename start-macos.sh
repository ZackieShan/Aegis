...
PORT="${AEGIS_PORT:-${APP_PORT:-7860}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
    ...
    echo "    AEGIS_PORT=7900 ./start-macos.sh"
    exit 1
fi
...
if [ "$MACHINE_ARCH" = "arm64" ]; then
    if command -v apfel >/dev/null 2>&1; then
        APFEL_LOG="${TMPDIR:-/tmp}/aegis-apfel.log"
...
# AEGIS_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$AEGIS_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
    (
