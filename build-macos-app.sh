...
# so rebuild if you move the repo. Override the port with AEGIS_PORT.
set -e

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="Aegis"
INSTALL_DIR="$REPO_DIR"
PORT="${AEGIS_PORT:-7860}"
DIST="$REPO_DIR/dist"
...
# ── Icon (best effort) — center-crop docs/aegis.jpg to a square .icns ──
if [ -f "$REPO_DIR/docs/aegis.jpg" ] && command -v sips >/dev/null 2>&1; then
  TMPIMG="$(mktemp -d)"
  ...
  sips -c 720 720 "$REPO_DIR/docs/aegis.jpg" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$REPO_DIR/docs/aegis.jpg" "$TMPIMG/sq.png"
  ...
  echo "  icon:        (skipped — no docs/aegis.jpg)"
fi
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.aegis.launcher</string>
