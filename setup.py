...
def _prompt_admin_credentials():
    ...
    while True:
        ...
        if not password:
            # Default credentials — fine for a localhost-only install; the UI
            # nags to change it and the change-password flow enforces
            # PASSWORD_MIN_LENGTH on whatever replaces it.
            password = "admin"
            break
...
def create_default_admin():
    ...
    try:
        ...
        username = os.getenv("AEGIS_ADMIN_USER", "").strip().lower()
        password = os.getenv("AEGIS_ADMIN_PASSWORD", "").strip()

        if username and password:
            # Both provided via env — validate before using
            if username in RESERVED_USERNAMES:
                print(f"  [error] AEGIS_ADMIN_USER '{username}' is a reserved username")
                return "failed"
            if len(password) < PASSWORD_MIN_LENGTH:
                print(f"  [error] AEGIS_ADMIN_PASSWORD must be at least {PASSWORD_MIN_LENGTH} characters")
                return "failed"
        elif sys.stdin.isatty() and not os.getenv("AEGIS_SKIP_ADMIN_PROMPT"):
            # Interactive terminal — ask the user
            username, password = _prompt_admin_credentials()
        else:
            # Non-interactive (Docker, CI) — fall back to the default
            # credentials. Localhost-only by default; change after first login.
        ...
        if sys.stdin.isatty() and not os.getenv("AEGIS_ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
            if password == "admin":
                print(f"        Default login: {username} / admin")
                print(f"        ** Change it after first login (Settings -> Account). **")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not os.getenv("AEGIS_ADMIN_PASSWORD"):
                print(f"        Default login: {username} / {password}")
                print(f"        ** Change it after first login. Set AEGIS_ADMIN_PASSWORD to choose your own. **")
        return "created"
...
def main():
    ...
    # Load .env so pre-seeded AEGIS_ADMIN_USER / AEGIS_ADMIN_PASSWORD (and
    ...
    if not os.getenv("AEGIS_SKIP_RUN_HINT"):
