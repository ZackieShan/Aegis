...
This populates the `demo@aegis.local` Dovecot account (which has NO mbsync
...
USER = os.getenv("DEMO_IMAP_USER", "demo@aegis.local")
...
LOOKUP_MSGID = "<demo-lookup-deepseek@aegis.local>"
...
def _msg(*, frm, to=None, subject, text, html=None, days_ago=0, hours_ago=0,
    ...
    m["Message-ID"] = msg_id or make_msgid(domain="aegis.local")
    if in_reply_to:
...
def build_dataset() -> list[dict]:
    ...
    add("INBOX", "", _msg(
        frm="Greg <greg@aegis-demo.example>",
    ...
    orig_id = make_msgid(domain="aegis.local")
    add("Sent", "(\\Seen)", _msg(
    ...
    add("INBOX", "(\\Flagged)", _msg(
        frm="Ops Bot <ops@aegis-demo.example>",
        subject="[URGENT] prod is on fire 🔥 — aegis-ui 502s",
        days_ago=0, hours_ago=1,
        text=("PAGE: aegis-ui is returning 502s on the /api/chat endpoint.\n"
    ...
    try:
        ...
        con.execute(
            ...
            (LOOKUP_MSGID, "", "INBOX", "quick q for the slide — DeepSeek-V3 param count?",
             "greg@aegis-demo.example", summary, "demo", now))
        con.commit()
...
def _wipe(conn: imaplib.IMAP4) -> int:
    ...
    """
    safe_target = USER.endswith("@aegis.local") or HOST in ("localhost", "127.0.0.1", "::1")
