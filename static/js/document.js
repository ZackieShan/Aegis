...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'aegis.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__aegisActiveEmailAccount || null;
  ...
  // Inject tab menu styles immediately (must exist before any hover)
  {
    ...
    s.textContent = `.doc-tab-menu-btn{background:none!important;border:none!important;outline:none!important;box-shadow:none!important;color:var(--fg);opacity:0.25;cursor:pointer;padding:2px 4px!important;height:auto!important;line-height:1;transition:opacity .15s;flex-shrink:0;-webkit-appearance:none;appearance:none}.doc-tab-menu-btn:focus,.doc-tab-menu-btn:active{outline:none!important;box-shadow:none!important;background:none!important}.doc-tab:hover .doc-tab-menu-btn{opacity:.5}.doc-tab-menu-btn:hover{opacity:1!important}.doc-tab-dropdown .dropdown-item-compact{padding:6px 8px;border-radius:8px;cursor:pointer;white-space:nowrap;border-bottom:none;display:flex;align-items:center;gap:10px;font-size:11px}.doc-tab-dropdown .dropdown-item-compact:hover{background:color-mix(in srgb,var(--fg) 8%,transparent)}.doc-tab-dropdown .dropdown-item-compact .dropdown-icon{width:14px;height:14px;display:flex;align-items:center;justify-content:center;flex-shrink:0;opacity:0.5}.doc-tab-dropdown .dropdown-divider{height:1px;margin:3px 0;background:color-mix(in srgb,var(--border) 40%,transparent)}.doc-tab-action-delete{color:var(--red,#e06c75)!important}.doc-tab-action-delete .dropdown-icon{opacity:0.7!important}`;
  ...
  const _docOpenKey = (sessionId) => 'aegis-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'aegis-doc-minimized-' + sessionId;
  ...
  let _aegisAttachMenu = null;

  function _closeAegisAttachMenu() {
    if (_aegisAttachMenu) {
      _aegisAttachMenu.remove();
      _aegisAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_aegisAttachMenu && !_aegisAttachMenu.contains(e.target)) _closeAegisAttachMenu();
  }

  function _attachMenuEscape(e) {
    if (e.key !== 'Escape') return;
    _closeAegisAttachMenu();
  }

  function _positionAegisAttachMenu(anchor, menu) {
  ...
  async function _stageAegisAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-aegis`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageAegisZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-aegis-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterAegisAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachAegisItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageAegisAttachment(kind, id);
      if (!data) return;
      _afterAegisAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeAegisAttachMenu();
    } catch (err) {
  ...
  function _selectedAegisAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-aegis-attach-row.is-selected') || []);
  }

  function _syncAegisAttachSelection(menu) {
    const selected = _selectedAegisAttachRows(menu);
    const bar = menu?.querySelector?.('.email-aegis-attach-actions');
    const count = menu?.querySelector?.('.email-aegis-attach-count');
    const attachBtn = menu?.querySelector?.('.email-aegis-attach-selected');
  ...
  async function _attachSelectedAegisItems(menu) {
    const rows = _selectedAegisAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-aegis-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageAegisZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageAegisAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterAegisAttachmentsAdded(added, zip ? 'aegis-attachments.zip' : undefined);
      _closeAegisAttachMenu();
    } catch (err) {
  ...
  async function _loadAegisAttachItems(menu, kind) {
    const list = menu.querySelector('.email-aegis-attach-list');
    if (!list) return;
    ...
    const q = (menu.querySelector('.email-aegis-attach-search')?.value || '').trim();
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-aegis-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncAegisAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _aegisAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-aegis-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-aegis-attach-dot" aria-hidden="true"></span>
            <span class="email-aegis-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-aegis-attach-main">
              <span class="email-aegis-attach-title">${_escHtml(label)}</span>
              <span class="email-aegis-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-aegis-attach-dot" aria-hidden="true"></span>
            <span class="email-aegis-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-aegis-attach-main">
              <span class="email-aegis-attach-title">${_escHtml(label)}</span>
              <span class="email-aegis-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachAegisItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncAegisAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load Aegis attach items:', err);
      list.innerHTML = '<div class="email-aegis-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-aegis-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-aegis-attach-local">
      ...
      <div class="email-aegis-attach-tabs">
      ...
      <label class="email-aegis-attach-search-wrap">
        ...
        <input type="search" class="email-aegis-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-aegis-attach-list"></div>
      <div class="email-aegis-attach-actions">
        <span class="email-aegis-attach-count"></span>
        <button type="button" class="email-aegis-attach-selected" disabled>
    ...
    _aegisAttachMenu = menu;
    _positionAegisAttachMenu(anchor, menu);
    menu.querySelector('.email-aegis-attach-local')?.addEventListener('click', () => {
      _closeAegisAttachMenu();
      document.getElementById('doc-email-file-input')?.click();
    });
    menu.querySelectorAll('[data-ody-attach-kind]').forEach(btn => {
      btn.addEventListener('click', () => _loadAegisAttachItems(menu, btn.dataset.odyAttachKind));
    });
    let attachSearchTimer = null;
    menu.querySelector('.email-aegis-attach-search')?.addEventListener('input', () => {
      clearTimeout(attachSearchTimer);
      attachSearchTimer = setTimeout(() => {
        _loadAegisAttachItems(menu, menu.dataset.odyAttachKind || 'document');
      }, 220);
    });
    menu.querySelector('.email-aegis-attach-selected')?.addEventListener('click', () => _attachSelectedAegisItems(menu));
    setTimeout(() => {
      document.addEventListener('click', _attachMenuOutsideClick, true);
      document.addEventListener('keydown', _attachMenuEscape, true);
    }, 0);
    _loadAegisAttachItems(menu, 'document');
  }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__aegisActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'aegis:email-ai-reply-context:v1:';
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('aegis-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('aegis-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'aegis-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('aegis-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('aegis-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('aegis-suggestions-' + docId);
      if (!raw) return;
