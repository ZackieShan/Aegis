...
let _sortMode = Storage.get('aegis-session-sort') || 'active'; // default to last active
...
const FOLDER_STATE_KEY = 'aegis-folder-state';
const FOLDER_ORDER_KEY = 'aegis-folder-order';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  try {
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('aegis-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('aegis-session-sort', mode);
  else Storage.remove('aegis-session-sort');
  renderSessionList();
}
