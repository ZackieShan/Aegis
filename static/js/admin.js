...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('aegis:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  function _isLocalPresetSelected() {
    const opt = _selectedProviderOption();
    return !!(opt && opt.dataset && opt.dataset.local);
  }
  function _setApiFormForProvider() {
    ...
    if (deviceAuthConfig) {
      ...
      if (apiKey) {
        apiKey.placeholder = _isLocalPresetSelected() ? 'API key (optional for local servers)' : 'API key';
  ...
  provider.addEventListener('change', () => {
    ...
    if (kindSel) kindSel.value = _isLocalPresetSelected() ? 'local' : (provider.value ? 'api' : 'proxy');
    _setApiFormForProvider();
  });
  ...
  if (apiTestBtn) {
    apiTestBtn.addEventListener('click', async () => {
      ...
      if (provider.value && !apiKey && !_isLocalPresetSelected()) { msg.textContent = 'API key is required for cloud providers'; msg.className = 'admin-error'; return; }
  ...
  document.querySelectorAll('.adm-quickstart-section').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'aegis.addModels.' + sec.id + '.open';
    let open = false;
      ...
      return `<div class="admin-user-row" data-adm-mcp-id="${s.id}">
        <div style="display:flex;align-items:center;justify-content:space-between;${hasTools ? 'cursor:pointer;' : ''}padding:4px 0;" data-adm-mcp-header="${s.id}">
          <div class="admin-user-info" style="flex:1;flex-wrap:wrap;gap:0.3rem;">
            ...
            ${s.builtin ? `<span class="admin-badge" style="background:var(--accent,var(--red))22;color:var(--accent,var(--red));" title="Built-in toolbox">Built-in</span>` : ''}
          ...
          <div style="display:flex;gap:4px;align-items:center;">
            ...
            ${s.builtin ? '' : `<button class="admin-btn-delete" data-adm-mcp-delete="${s.id}">Delete</button>`}
  ...
  try {
    ...
    list.querySelectorAll('[data-adm-del-token]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try { window.dispatchEvent(new CustomEvent('aegis-integrations-changed')); } catch (_) {}
      });
    });
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'aegis_backup.json';
