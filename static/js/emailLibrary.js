...
document.addEventListener('aegis:email-filter-tag', (e) => {
...
try { window.__aegisGetActiveEmailContext = _getActiveEmailContext; } catch (_) {}
...
window.addEventListener('aegis-reminder-channel-changed', (e) => {
...
function _isChatInteractionBusy() {
  try {
    if (window.__aegisChatBusy) return true;
    const until = Number(window.__aegisChatBusyUntil || 0);
...
function _publishActiveAccount() {
  try { window.__aegisActiveEmailAccount = state._libAccountId || null; } catch (_) {}
...
export function openEmailLibrary(opts = {}) {
  ...
  if (tagsToggle) {
    ...
    tagsToggle.addEventListener('click', () => {
      ...
      localStorage.setItem('aegis.email.showTags', state._libShowTags ? '1' : '0');
      tagsToggle.classList.toggle('active', !!state._libShowTags);
      tagsToggle.setAttribute('aria-pressed', String(!!state._libShowTags));
      _renderGrid();
      document.dispatchEvent(new CustomEvent('aegis:email-tags-toggle', { detail: { show: state._libShowTags } }));
    });
  }
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    ...
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/email/aegis/reminders?permanent=1${_acct()}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
...
const _BUBBLES_DISABLED_KEY = 'aegis.email.bubblesDisabled';
...
const _SUMMARY_COLLAPSED_KEY = 'aegis.email.summaryCollapsed';
