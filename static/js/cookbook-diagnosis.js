...
if command -v nvidia-smi >/dev/null 2>&1; then
  ...
  if [ -n "$alive" ]; then
    echo "[aegis] Force killing remaining GPU PIDs:$alive"
  ...
  if [ -n "$remaining" ]; then
    echo "[aegis] GPU processes still remain:"
    echo "$remaining"
    exit 2
  fi
  echo "[aegis] GPU cleanup complete. No NVIDIA compute processes remain."
else
  echo "[aegis] nvidia-smi not found; falling back to common model-server process cleanup."
  ...
  echo "[aegis] Fallback cleanup complete."
fi`;
}
