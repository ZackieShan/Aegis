...
function _isForegroundChatBusy() {
  ...
  return !!window.__aegisChatBusy
    || Date.now() < (window.__aegisChatBusyUntil || 0)
...
function _submitMobileQueuedInput(input) {
  ...
  window.__aegisQueueStreamingSubmit = now;
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__aegisDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (_opener) window._aegisRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('aegis-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModeToggle() {
    ...
    window.__aegisSetChatMode = setMode;
  ...
  const SPLASH_COUNT_KEY = 'aegis-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__aegisSetChatMode === 'function') {
          window.__aegisSetChatMode('chat');
        } else {
        ...
        if (typeof window.__aegisSetChatMode === 'function') {
          window.__aegisSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'aegis-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'aegis-toolbar-visibility';
    try {
  ...
  if (textarea) {
    ...
    window.addEventListener('aegis:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__aegisQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
function startAegisApp() {
  if (window.__aegisAppStarted) return;
  window.__aegisAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__aegisChatBusyUntil = Math.max(window.__aegisChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('aegis-doc-open-' + _curSession) === '1') {
  ...
  const _railToolMap = {
    ...
    'rail-photos':    'tool-photos-btn',
    'rail-cinema':    'tool-cinema-btn',
    'rail-music':     'tool-music-btn',
  ...
  window._aegisBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      if (sendBtn.dataset.mode === 'streaming') {
        if (hasText) window.__aegisQueueStreamingSubmit = Date.now();
        handleSubmit(e);
        return;
      }
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._aegisRouteOpener) {
          try { window._aegisRouteOpener(); } catch (_) {}
          window._aegisRouteOpener = null;
        }
      });
  } else {
  ...
  const runNonCriticalStartup = (fn, delay = 4000) => {
    let tries = 0;
    const run = () => {
      const busy = !!window.__aegisChatBusy
        || Date.now() < (window.__aegisChatBusyUntil || 0)
        || !!document.querySelector('.send-btn[data-mode="streaming"], .send-btn.send-pending');
      if (busy && tries < 12) {
        tries += 1;
