...
# Plain "bash" on Windows can resolve to the System32 WSL stub; find_bash
# locates a real interpreter (e.g. Git Bash) and skips the stub.
_BASH = find_bash() or "bash"
...
def test_vllm_preflight_reports_cli_and_version():
    ...
    assert 'AEGIS_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"' in script
    assert 'echo "[aegis] vLLM CLI: $AEGIS_VLLM_BIN"' in script
    assert '"$AEGIS_VLLM_BIN" --version' in script
    assert 'AEGIS_PREFLIGHT_EXIT=127' in script
...
def _bash_has_python3() -> bool:
    """True when `python3` inside _BASH actually runs (on Windows the name
    often resolves to the Microsoft Store stub, which exits non-zero)."""
    try:
        probe = subprocess.run(
            [_BASH, "-c", "python3 -c 'pass'"], capture_output=True, timeout=15
        )
    except Exception:
        return False
...
@pytest.mark.skipif(not _bash_has_python3(), reason="no working python3 in bash")
...
def test_serve_preflight_failure_keeps_tmux_pane_visible():
    ...
    assert 'AEGIS_PREFLIGHT_EXIT=127' in script
    assert 'echo "=== Process exited with code $AEGIS_PREFLIGHT_EXIT ==="' in script
...
def test_serve_runner_preserves_command_exit_code():
    ...
    assert "AEGIS_CMD_EXIT=$?" in script
    assert 'echo "=== Process exited with code $AEGIS_CMD_EXIT ==="' in script
...
def test_pip_serve_runner_emits_download_ok_before_exit_marker():
    ...
    assert 'exit "$AEGIS_CMD_EXIT"' in script
...
def test_llama_cpp_linux_bootstrap_checks_cudart_before_cuda_build():
    ...
    assert '_aegis_has_cudart' in script
    ...
    assert script.index('_aegis_has_cudart') < script.index('DGGML_CUDA=ON')
...
def test_local_windows_download_pid_is_a_windows_pid_and_stop_kills_tree():
    ...
    # The pid file must hold the Windows pid of the outer Git Bash, written by
    # Python after Popen — never "$$" from inside the script. Under Git Bash,
    # $$ is the MSYS-internal pid, invisible to pid_alive()'s OpenProcess
    # probe and to taskkill, so the poller marked live downloads as crashed
    # while pip/hf were still running. taskkill /T (kill_process_tree) and the
    # client's Stop-Tree walk from the outer pid still reach the whole tree.
    assert 'printf \'%s\\\\n\' \\"$$\\" > {pp}' not in routes_src
    assert "pid_path.write_text(str(proc.pid)" in routes_src
...
def test_cached_model_scan_reports_loose_gguf_at_dir_root(tmp_path):
    """A bare .gguf dropped directly in a model dir (not in a subfolder) — the
    natural 'drop a model here' gesture — must show up as its own single-file
    model named after the file, so the models/ drop folder just works."""
    (tmp_path / "my-model-Q4_K_M.gguf").write_bytes(b"gguf")
    (tmp_path / "another.Q6_K.gguf").write_bytes(b"ggufgguf")
    (tmp_path / "notes.txt").write_bytes(b"ignore me")  # non-gguf ignored

    scan_py = tmp_path / "scan_cache.py"
    scan_py.write_text(_cached_model_scan_script([str(tmp_path)]), encoding="utf-8")
    proc = subprocess.run(
        [sys.executable, str(scan_py)], check=True, capture_output=True, text=True
    )

    by_repo = {m["repo_id"]: m for m in json.loads(proc.stdout)}
    assert "my-model-Q4_K_M" in by_repo
    assert by_repo["my-model-Q4_K_M"]["is_gguf"] is True
    assert by_repo["my-model-Q4_K_M"]["is_local_dir"] is True
    assert [f["name"] for f in by_repo["my-model-Q4_K_M"]["gguf_files"]] == ["my-model-Q4_K_M.gguf"]
    assert "another.Q6_K" in by_repo
...
def test_cached_model_scan_uses_ollama_api_before_cli_and_windows_opt_in():
    ...
    assert "AEGIS_ALLOW_OLLAMA_CLI_SCAN" in script
...
def test_cached_model_scan_does_not_launch_ollama_cli_on_windows(tmp_path):
    ...
    env.pop("AEGIS_ALLOW_OLLAMA_CLI_SCAN", None)
    # Isolate from any Ollama actually running on this dev machine: the scan
    # also probes the HTTP API, which would report real local models here.
