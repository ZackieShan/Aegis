...
def _load_setup_module():
    spec = importlib.util.spec_from_file_location("aegis_setup_under_test", Path("setup.py"))
...
def test_create_default_admin_normalizes_env_username(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AEGIS_ADMIN_USER", " AdminUser ")
    monkeypatch.setenv("AEGIS_ADMIN_PASSWORD", "temporary-password")
...
def test_create_default_admin_defaults_to_admin_admin_when_non_interactive(tmp_path, monkeypatch):
    """Fresh non-interactive installs (Docker, launcher scripts) seed the
    default admin/admin login instead of a random password, so the quickstart
    'log in with admin / admin' instruction holds."""
    import bcrypt

    setup_module = _load_setup_module()
    monkeypatch.setattr(setup_module, "AUTH_FILE", str(tmp_path / "auth.json"))
    monkeypatch.delenv("AEGIS_ADMIN_USER", raising=False)
    monkeypatch.delenv("AEGIS_ADMIN_PASSWORD", raising=False)
    monkeypatch.setenv("AEGIS_SKIP_ADMIN_PROMPT", "1")
...
def test_main_loads_admin_password_from_env_file(tmp_path, monkeypatch):
    ...
    (tmp_path / ".env").write_text(
        "AEGIS_ADMIN_USER=presetuser\nAEGIS_ADMIN_PASSWORD=fromenvfile12345\n",
        encoding="utf-8-sig",
    )
    ...
    try:
        ...
        # don't inherit them.
        os.environ.pop("AEGIS_ADMIN_USER", None)
        os.environ.pop("AEGIS_ADMIN_PASSWORD", None)
