...
def test_readme_opens_with_wordmark_title():
    # The README must still open with a recognizable Aegis title: a centered
    # H1 (or wordmark image) rather than an un-fenced ASCII banner.
    head = "\n".join(README.read_text(encoding="utf-8").splitlines()[:15])
    assert 'alt="Aegis"' in head or ">Aegis</h1>" in head, (
        "README must open with the Aegis wordmark image or an Aegis H1 title"
    )
