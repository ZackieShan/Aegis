...
def _compose_env_names(path: Path) -> set[str]:
    ...
    env = compose["services"]["aegis"]["environment"]
...
def _upload_limit_env_names() -> set[str]:
    ...
    return set(re.findall(r'"(AEGIS_[A-Z_]*BYTES)"', source)) | {
        "AEGIS_CHAT_UPLOAD_MAX_BYTES"
    }
...
def test_host_docker_overlay_mounts_socket_and_adds_docker_group():
    ...
    service = overlay["services"]["aegis"]
