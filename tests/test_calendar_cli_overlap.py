"""Regression: `aegis-calendar list` must select events that OVERLAP the
...
`aegis-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "aegis-calendar"
    loader = importlib.machinery.SourceFileLoader("aegis_calendar_cli", str(path))
