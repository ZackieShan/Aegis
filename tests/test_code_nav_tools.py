...
os.environ.setdefault(
    "DATABASE_URL",
    "sqlite:///" + os.path.join(tempfile.gettempdir(), "test_code_nav.db").replace("\\", "/"),
)
...
def repo():
    ...
    # allowlist. Yielded with forward slashes: the paths get embedded into
    # JSON tool payloads below, where Windows backslashes would be parsed as
...
def test_read_file_offset_limit(repo):
    # Forward-slash join: this path is embedded in a JSON payload, where a
    # Windows backslash would be parsed as a (broken) escape sequence.
    p = f"{repo}/lines.txt"
    with open(p, "w") as f:
