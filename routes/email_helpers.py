...
def _friendly_email_auth_error(protocol: str, host: str, error: object) -> str:
    ...
    if microsoft_basic_auth_failure:
        return (
            ...
            "Outlook/Office 365 IMAP/SMTP in most accounts. Aegis "
...
# Override at deploy time via AEGIS_MAIL_ATTACHMENTS_DIR. Defaults to a
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "aegis_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN aegis_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("AEGIS_IMAP_TIMEOUT_SECONDS"))
