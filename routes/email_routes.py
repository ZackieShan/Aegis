...
def _utcnow() -> datetime:
    """Naive UTC to keep the ISO strings this module stores/compares in the
    same format as before, without deprecated _utcnow()."""
    return datetime.now(timezone.utc).replace(tzinfo=None)


AEGIS_MAIL_ORIGIN = "aegis-ui"
    ...
    try:
        ...
        try:
            ...
            conn.execute(
                ...
                (" ".join(sorted(parts)), _utcnow().isoformat() + "Z", owner or "", _account_cache_key(account_id, owner), folder, str(uid)),
            )
            conn.commit()
        finally:
            conn.close()
...
def _apply_aegis_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-Aegis-Origin"] = AEGIS_MAIL_ORIGIN
    if kind:
        msg["X-Aegis-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-Aegis-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _fixture_email_record(row: dict, uid_num: int, owner: str) -> dict:
        ...
        return {
            "uid": uid,
            "message_id": f"<fixture-email-{uid}-{owner_key}@fixtures.aegis.local>",
            "subject": subject,
    ...
    def _fixture_email_list(folder: str, limit: int, offset: int, filter_: str, from_addr: str | None, owner: str) -> dict | None:
        ...
        return {
            ...
            "total": total,
            "folder": folder,
            "sync": {"source": "fixture", "updated_at": _utcnow().isoformat() + "Z"},
        }
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # fallback too. The fallback uses a distinct Aegis prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-Aegis-Kind "reminder" SUBJECT "Reminder (Aegis):"{from_clause})',
                )
                ...
                _since = (_utcnow() - _td(days=30)).strftime("%d-%b-%Y")
                ...
                from datetime import timedelta as _td
                _before = (_utcnow() - _td(days=30)).strftime("%d-%b-%Y")
    ...
    @router.delete("/aegis/reminders")
    async def delete_aegis_reminder_emails(
        account_id: str | None = Query(None),
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-Aegis-Kind {_search_quote("reminder")})'))
                        ...
                        for addr in own_addrs:
                            ...
                            # Legacy reminders created before the Aegis
            ...
            logger.error(f"delete_aegis_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
        ...
        try:
            ...
            for f in folders:
                ...
                if match:
                    ...
                    result.append(name)
            payload = {"folders": result, "sync": {"source": "imap", "updated_at": _utcnow().isoformat() + "Z"}}
    ...
    def _load_aegis_attachment_source(db, kind: str, item_id: str, owner: str):
    ...
    @router.post("/compose-from-aegis")
    async def compose_from_aegis(data: dict, owner: str = Depends(require_owner)):
        """Stage an Aegis document or gallery image as a compose upload."""
        ...
        try:
            ...
            db = _SL()
            try:
                src = _load_aegis_attachment_source(db, kind, item_id, owner)
                if "path" in src:
            ...
            logger.error(f"Failed to stage Aegis attachment {kind}/{item_id}: {e}")
            return {"success": False, "error": "Mail operation failed"}

    @router.post("/compose-from-aegis-zip")
    async def compose_from_aegis_zip(data: dict, owner: str = Depends(require_owner)):
        """Stage several Aegis documents/gallery images as one zip attachment."""
        ...
        try:
            ...
            try:
                ...
                return _stage_compose_bytes("aegis-attachments.zip", content)
            finally:
                db.close()
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Failed to stage Aegis zip attachment: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", aegis_kind=None, aegis_ref=None,
    ):
        ...
        _apply_aegis_headers(outer, aegis_kind or "scheduled", aegis_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            now_utc = _dt.now(_tz.utc) if parsed_at.tzinfo else _utcnow()
            ...
            # _utcnow().isoformat(), so storing the raw client string
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, aegis_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("aegis_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def approve_agent_draft(sid: str, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            cur = conn.execute(
                ...
                (_utcnow().isoformat(), sid, owner or ""),
            )
            conn.commit()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = _utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="aegis.local")
        ...
        if req.aegis_kind:
            _apply_aegis_headers(outer, req.aegis_kind)
    ...
    async def translate_email(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            if "<<<SAME_LANGUAGE>>>" in content:
                try:
                    ...
                    """, (
                        ...
                        subject, sender, "", 1, model, _utcnow().isoformat(),
                    ))
                    _c.commit()
                    _c.close()
                except Exception as e:
            ...
            try:
                ...
                """, (
                    body_hash, owner, target_language, data.get("uid", ""), data.get("folder", ""),
                    subject, sender, content, 0, model, _utcnow().isoformat(),
                ))
                _c.commit()
                _c.close()
            except Exception as e:
            ...
            if message_id:
                try:
                    ...
                    _c.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (message_id, owner, source_uid, source_folder, reply, model, _utcnow().isoformat()))
                    _c.commit()
                    _c.close()
                except Exception as e:
