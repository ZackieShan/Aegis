...
from core.platform_compat import find_bash, run_ssh_command
...
def _run_model_probe(host: str, ssh_port: str, cmd: str) -> str:
    try:
        if host:
            ...
            # find_bash() locates a real interpreter (Git Bash on Windows);
            # plain "bash" can resolve to the System32 WSL stub, which fails
            # with a WSL relay error when no distro is provisioned.
            bash = find_bash()
            if not bash:
                return ""
