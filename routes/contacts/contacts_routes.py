...
def _utcnow() -> datetime:
    """Naive UTC for the contact-cache timestamps, without deprecated
    _utcnow()."""
...
def _save_local_contacts(contacts: List[Dict]) -> None:
    ...
    _contact_cache["fetched_at"] = _utcnow()
...
def _fetch_contacts(force=False):
    ...
    if not force and _contact_cache["fetched_at"]:
        age = (_utcnow() - _contact_cache["fetched_at"]).total_seconds()
        if age < 60:
...
def setup_contacts_routes():
    ...
    async def export_contacts(
        ...
        if format == "csv":
            ...
            filename = "aegis-contacts.csv"
        else:
            content = _contacts_to_vcf(contacts)
            media_type = "text/vcard; charset=utf-8"
            filename = "aegis-contacts.vcf"
        return Response(
            content=content,
