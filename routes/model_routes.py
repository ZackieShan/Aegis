...
_ENDPOINT_SETTING_FIELDS = {
    ...
    "coder_endpoint_id":    ("coder_model",      "Coder Model"),
}
...
def _container_loopback_reachable(base_url: str, timeout: float = 0.2) -> bool:
    ...
    This distinguishes "a model server running alongside Aegis in the same
...
def _rewrite_loopback_for_docker(base_url: str, *, container_local: bool = False) -> str:
    ...
    Cookbook local serves are the opposite case: Aegis started the model
...
def _is_mlx_deepseek_v4_shim_id(model_id: str) -> bool:
    m = str(model_id or "").lower()
    return "/.cache/aegis/mlx-shims/deepseek-v4" in m
...
def _filter_mlx_deepseek_v4_repo_when_shimmed(model_ids):
    ...
    only works through Aegis' sanitized local shim. Keep the shim as the
...
def setup_model_routes(model_discovery):
    ...
    def api_models(request: Request, refresh: bool = False, background: bool = False):
        ...
        # Page boot can opt out with background=false so opening Aegis does
    ...
    def create_model_endpoint(
        ...
        # server. Cookbook local serves are launched inside Aegis itself, so
        ...
        try:
            ...
            # Local Ollama endpoints support native tool-calling (verified for
            # gemma/qwen on modern Ollama), but leaving the flag unknown routes
            # to the text tool path where Ollama models don't reliably emit a
            # parseable tool call — so toolboxes silently never fire. Default it
            # ON for Ollama so tools work out of the box; the admin can still
            # toggle it off per endpoint.
            if _st is None:
                _bl = (base_url or "").lower()
                if "11434" in _bl or "ollama" in _bl or _bl.rstrip("/").endswith("/api"):
                    _st = True
