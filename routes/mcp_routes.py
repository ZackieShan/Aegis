...
def _mcp_oauth_base_dir() -> Path:
    """Directory that may contain OAuth files managed by Aegis."""
...
def setup_mcp_routes(mcp_manager: McpManager):
        ...
        try:
            ...
            for srv in servers:
                ...
                result.append({
                    "id": srv.id,
                    "name": srv.name,
                    "builtin": srv.id in BUILTIN_MCP_IDS,
    ...
    async def delete_server(server_id: str, request: Request):
        ...
        if server_id in BUILTIN_MCP_IDS:
            raise HTTPException(400, "Built-in toolboxes can't be deleted — disable it instead.")
        db = SessionLocal()
        try:
            srv = db.query(McpServer).filter(McpServer.id == server_id).first()
            if not srv:
    ...
    async def oauth_callback(code: str, state: str, request: Request):
        ...
        if resolve_pending(state, code):
            return HTMLResponse(_oauth_result_page(
                ...
                "The MCP server is connecting. You can close this window and return to Aegis.",
                success=True,
            ))
