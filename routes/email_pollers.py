...
def _utcnow() -> datetime:
    """Naive UTC to keep this module's stored/compared ISO strings in the
                    ...
                    try:
                        ...
                        if summary:
                            ...
                            _c.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), _folder, subject, sender, summary, model, _utcnow().isoformat()))
                            _c.commit()
                            _c.close()
                    ...
                    try:
                        ...
                        if reply:
                            ...
                            _c.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?)
                            """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), _folder, reply, model, _utcnow().isoformat()))
                            _c.commit()
                            _c.close()
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "aegis_kind" if "aegis_kind" in cols else "'scheduled' AS aegis_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                aegis_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Aegis-Origin"] = "aegis-ui"
                outer["X-Aegis-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", aegis_kind or "scheduled")[:64]
                outer["X-Aegis-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `aegis-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `AEGIS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `aegis-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("AEGIS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `AEGIS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (AEGIS_INPROCESS_POLLERS=0); "
            "drive `aegis-mail poll-scheduled` externally."
        )
        return
    import asyncio
