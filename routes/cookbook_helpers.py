...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends Aegis' own interpreter directory to PATH. If Aegis itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[aegis] pip does not support --break-system-packages; installing without it."')
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'AEGIS_USER_SHELL="${SHELL:-}"',
        'if [ -n "$AEGIS_USER_SHELL" ] && [ -x "$AEGIS_USER_SHELL" ]; then',
        '  AEGIS_USER_PATH="$("$AEGIS_USER_SHELL" -ic \'printf "__AEGIS_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__AEGIS_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$AEGIS_USER_PATH" ]; then export PATH="$AEGIS_USER_PATH:$PATH"; fi',
        'fi',
...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "    return files",
        "def collect_ggufs_shallow(base):",
        "    # GGUFs sitting directly in `base` (top level only) — subfolders are",
        "    # handled as their own models by scan_dir, so don't recurse here.",
        "    files = []",
        "    split_groups = {}",
        "    if not os.path.isdir(base) or not safe_path(base): return files",
        ...
        "    for fn in entries:",
        "        if not fn.lower().endswith('.gguf') or fn.startswith('._'): continue",
        ...
        "        if sm:",
        "            prefix, part_s, total_s = sm.group(1), sm.group(2), sm.group(3)",
        "            g = split_groups.setdefault((prefix, total_s), {'name':fn,'rel_path':fn,'size_bytes':0,'role':gguf_role(fn),'quant':gguf_quant(fn),'parts':int(total_s),'split':True})",
        "            g['size_bytes'] += size",
        "            if int(part_s) == 1: g.update({'name':fn,'rel_path':fn,'role':gguf_role(fn),'quant':gguf_quant(fn)})",
        "            continue",
        "        files.append({'name':fn,'rel_path':fn,'size_bytes':size,'role':gguf_role(fn),'quant':gguf_quant(fn)})",
        "    files.extend(split_groups.values())",
        "    return [f for f in files if f.get('role') == 'model' or gguf_role(f['name']) == 'model']",
        "def scan_hf(cache):",
        ...
        # Loose .gguf files dropped directly in the scan dir (not in a subfolder)
        # — the natural 'drop a model here' gesture. Each standalone file (or
        # split set) becomes its own single-file model named after the file, so
        # `models/foo.gguf` shows up in Serve without needing a subfolder.
        "    loose = collect_ggufs_shallow(p)",
        "    for g in loose:",
        "        stem = re.sub(r'(?i)\\.gguf$', '', g['name'])",
        "        stem = re.sub(r'(?i)-\\d+-of-\\d+$', '', stem)",
        "        if stem in seen: continue",
        "        seen.add(stem)",
        "        models.append({'repo_id':stem,'size_bytes':g['size_bytes'],'nb_files':1,'has_incomplete':False,'path':p,'is_local_dir':True,'is_diffusion':False,'is_gguf':True,'gguf_files':[g]})",
        ...
        "    if os.name == 'nt' and not os.environ.get('AEGIS_ALLOW_OLLAMA_CLI_SCAN'): return",
        ...
        "def scan_ollama_api():",
        "    # Honor the standard OLLAMA_HOST override (host[:port] or full URL)",
        "    # so non-default Ollama binds are scanned — and tests can isolate",
        "    # themselves from a live local Ollama by pointing this at a dead port.",
        "    _oh = os.environ.get('OLLAMA_HOST', '').strip()",
        "    if _oh:",
        "        if '://' not in _oh: _oh = 'http://' + _oh",
        "        urls = [_oh.rstrip('/') + '/api/tags']",
        "    else:",
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$AEGIS_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $AEGIS_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$AEGIS_PREFLIGHT_EXIT"')
    runner_lines.append('fi')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for Aegis-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('AEGIS_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$AEGIS_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  AEGIS_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[aegis] vLLM CLI: $AEGIS_VLLM_BIN"')
    runner_lines.append('  AEGIS_VLLM_VERSION="$("$AEGIS_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$AEGIS_VLLM_VERSION" ]; then echo "[aegis] vLLM version: $AEGIS_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('AEGIS_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $AEGIS_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $AEGIS_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $AEGIS_CMD_EXIT ==="')
        runner_lines.append('exit "$AEGIS_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # every OS-package dep from the launch path. Sets _aegis_have_prebuilt=1
    ...
    runner_lines.append('    _aegis_have_prebuilt=""')
    runner_lines.append('    _aegis_arch="$(uname -m)"')
    runner_lines.append('    _aegis_prebuilt_url=""')
    runner_lines.append('    if command -v curl >/dev/null 2>&1 && [ "$_aegis_arch" = "x86_64" ]; then')
    runner_lines.append('      _aegis_pat=""')
    runner_lines.append('      _aegis_has_nv_inline() { command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU "; }')
    runner_lines.append('      _aegis_has_vk_inline() { ldconfig -p 2>/dev/null | grep -q "libvulkan\\.so" || command -v vulkaninfo >/dev/null 2>&1 || [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so.1 ]; }')
    runner_lines.append('      _aegis_has_vkdev_inline() { ls /dev/dri/renderD* >/dev/null 2>&1 || (lspci 2>/dev/null | grep -Ei \'VGA|3D|Display\' | grep -Eiq \'AMD|ATI|Radeon\'); }')
    runner_lines.append('      if _aegis_has_nv_inline; then')
    runner_lines.append('        _aegis_pat="ubuntu.*cuda"')
    runner_lines.append('      elif _aegis_has_vkdev_inline && _aegis_has_vk_inline; then')
    runner_lines.append('        _aegis_pat="ubuntu.*vulkan"')
    runner_lines.append('      else')
    runner_lines.append('        _aegis_pat="ubuntu-x64\\\\.zip"')
    runner_lines.append('      fi')
    runner_lines.append('      _aegis_prebuilt_url="$(curl -fsSL --max-time 15 https://api.github.com/repos/ggml-org/llama.cpp/releases/latest 2>/dev/null | grep \'"browser_download_url"\' | cut -d\'"\' -f4 | grep -iE "$_aegis_pat" | grep -iv "arm\\|aarch64" | head -1)"')
    runner_lines.append('    fi')
    ...
    runner_lines.append('    if [ -n "$_aegis_prebuilt_url" ] && (command -v unzip >/dev/null 2>&1 || command -v bsdtar >/dev/null 2>&1 || command -v python3 >/dev/null 2>&1); then')
    runner_lines.append('      echo "[aegis] Found prebuilt llama-server: $_aegis_prebuilt_url"')
    runner_lines.append('      mkdir -p ~/bin "$HOME/.cache/aegis/llama-cpp-prebuilt" && cd "$HOME/.cache/aegis/llama-cpp-prebuilt"')
    runner_lines.append('      rm -f llama-cpp.zip')
    runner_lines.append('      if curl -fsSL --max-time 120 "$_aegis_prebuilt_url" -o llama-cpp.zip && [ -s llama-cpp.zip ]; then')
    ...
    runner_lines.append('        _aegis_extracted="$(find build -type f -name llama-server 2>/dev/null | head -1)"')
    runner_lines.append('        if [ -n "$_aegis_extracted" ]; then')
    runner_lines.append('          chmod +x "$_aegis_extracted"')
    runner_lines.append('          ln -sf "$_aegis_extracted" ~/bin/llama-server')
    runner_lines.append('          _aegis_libdir="$(dirname "$_aegis_extracted")"')
    runner_lines.append('          mkdir -p ~/.config && echo "export LD_LIBRARY_PATH=\\"$_aegis_libdir:\\${LD_LIBRARY_PATH:-}\\"" > ~/.config/aegis-llama-cpp-env')
    runner_lines.append('          _aegis_have_prebuilt=1')
    runner_lines.append('          echo "[aegis] Prebuilt llama-server installed at $_aegis_extracted"')
    runner_lines.append('        fi')
    runner_lines.append('      fi')
    runner_lines.append('      [ -z "$_aegis_have_prebuilt" ] && echo "[aegis] Prebuilt download/extract failed — falling back to from-source build."')
    runner_lines.append('    elif [ -z "$_aegis_prebuilt_url" ]; then')
    runner_lines.append('      echo "[aegis] No matching prebuilt llama-server for this host (arch=$_aegis_arch) — will build from source."')
    runner_lines.append('    fi')
    runner_lines.append('  if [ -z "$_aegis_have_prebuilt" ]; then')
    ...
    runner_lines.append('    _aegis_has_nvidia_hw() {')
    ...
    runner_lines.append('    }')
    runner_lines.append('    if _aegis_has_nvidia_hw; then')
    ...
    runner_lines.append('    _aegis_apt_bootstrap() {')
    ...
    runner_lines.append('        echo "[aegis] Auto-installing missing build deps via apt:$_missing"')
    ...
    runner_lines.append('        echo "[aegis] Auto-installing missing build deps via pacman:$_missing"')
    ...
    runner_lines.append('        echo "[aegis] Auto-installing missing build deps via dnf:$_missing"')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[aegis] WARNING: missing build deps ($_missing) — passwordless sudo is unavailable, cannot auto-install. Cookbook Diagnosis will explain the fix after the build fails."')
    runner_lines.append('      fi')
    runner_lines.append('    }')
    runner_lines.append('    _aegis_apt_bootstrap')
    runner_lines.append('    _aegis_missing_build_deps=""')
    runner_lines.append('    command -v cmake >/dev/null 2>&1 || _aegis_missing_build_deps="$_aegis_missing_build_deps cmake"')
    runner_lines.append('    command -v git >/dev/null 2>&1 || _aegis_missing_build_deps="$_aegis_missing_build_deps git"')
    runner_lines.append('    command -v g++ >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || _aegis_missing_build_deps="$_aegis_missing_build_deps build-essential"')
    runner_lines.append('    if [ -n "$_aegis_missing_build_deps" ]; then')
    runner_lines.append('      echo "ERROR: llama.cpp source build needs missing packages:$_aegis_missing_build_deps"')
    ...
    runner_lines.append('    _aegis_has_vulkan() {')
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _aegis_has_vulkan_device() {')
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[aegis] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('    elif command -v nvcc &>/dev/null && _aegis_has_nvidia_hw; then')
    runner_lines.append('      rm -rf build')
    ...
    runner_lines.append('      _aegis_has_cudart() {')
    ...
    runner_lines.append('        return 1')
    runner_lines.append('      }')
    runner_lines.append('      if _aegis_has_cudart; then')
    runner_lines.append('        echo "[aegis] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[aegis] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[aegis]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[aegis]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    elif _aegis_has_vulkan_device && _aegis_has_vulkan; then')
    runner_lines.append('      echo "[aegis] Vulkan-capable GPU detected (no ROCm/CUDA toolchain installed) — building llama-server with Vulkan support..."')
    ...
    runner_lines.append('    else')
    runner_lines.append('      echo "[aegis] WARNING: no HIP/CUDA/Vulkan toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[aegis]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[aegis]   Install Vulkan (libvulkan-dev) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
    runner_lines.append('  fi  # end _aegis_have_prebuilt guard')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    if update_source:
        update_cmd = (
            ...
            'echo "[aegis] WARNING: llama.cpp source update failed; clearing cached build anyway."; '
            'elif command -v git >/dev/null 2>&1; then '
            'git clone --depth 1 https://github.com/ggml-org/llama.cpp "$HOME/llama.cpp" || '
            'echo "[aegis] WARNING: llama.cpp clone failed; clearing cached build anyway."; '
            'fi && '
        )
    return (
        'mkdir -p "$HOME/bin" && '
        ...
        'echo "[aegis] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\aegis-sessions"
