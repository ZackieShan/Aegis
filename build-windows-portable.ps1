...
<#
  Build a portable Windows distribution for Aegis.

  Output layout:
    dist\Aegis\Aegis.exe
    dist\Aegis\static\...
    dist\Aegis\scripts\...
    dist\Aegis\mcp_servers\...
    dist\Aegis\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Aegis @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Aegis" -ForegroundColor Green
