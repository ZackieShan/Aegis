...
Aegis keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/aegis-backup` tool
...
```bash
# Create a snapshot → backups/aegis-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/aegis-backup snapshot

# List existing snapshots (most recent first)
./scripts/aegis-backup list

# Check a tarball's integrity without extracting it
./scripts/aegis-backup verify backups/aegis-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/aegis-backup restore backups/aegis-backup-20260101-120000.tar.gz --yes
```
...
./scripts/aegis-backup snapshot --out /mnt/nas/aegis-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/aegis-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/aegis && ./scripts/aegis-backup snapshot --out "/mnt/nas/aegis-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `aegis-backup` therefore does not capture the Docker
